...
namespace RIGIDT
{
 ...
 void update_scaffold(SCAFData &s)
{
  s.mv_num = s.m_V.rows();
  s.mf_num = s.m_T.rows();
   ...
   // std::vector<int> degen;
    for (int i = 0; i < M.size(); i++)
        if (M(i) < area_threshold )
            ...
            //for (int j = 0; j < 3; j++) {
            //    if (A(i, j) < igl::PI / 9|| A(i, j) >  igl::PI*8 / 9)
            //        degen.push_back(i);
            //}
        }
                                      ...
                                      Eigen::SparseMatrix<double> &D2)
{
  ...
  double area_threshold = min_bnd_edge_len /4;
  Eigen::SparseMatrix<double> Dx, Dy, Dz;
  std::vector<int> degen;
  adjusted_grad(V, F_s, area_threshold, Dx, Dy, Dz,degen);
  //for (auto i : degen) {
  //    s.s_M(i) *= 10;
  //}
 ...
 void mesh_improve(SCAFData &s)
{
 ...
 void add_new_patch(SCAFData &s, const Eigen::MatrixXd &V_ref,
                   ...
                   const Eigen::MatrixXd &uv_init)
{
 ...
 //s.component_sizes.push_back(F_ref.rows());
  ...
  for (int i = 0; i < M.size(); i++) {
      M(i) = pow(M(i), 0.3) + pow(M(i), -0.3);
  }
  s.m_M.bottomRows(M.size()) =  M / 2;

  for (auto cur_bnd : s.all_bnds)
  {
    ...
    int dim)
{
    ...
    Eigen::MatrixXd  eperp21(m, 3), eperp13(m, 3);
    ...
    for (int i = 0; i < m; ++i)
    {
        ...
        if (!uniform) {
            ...
            Eigen::Matrix<double, 3, 1> v1, v2, v3;
            v1 << 0, 0, 0;
            v2 << h, 0, 0;
            v3 << h / 2., (sqrt(3) / 2.)* h, 0;
    ...
    for (int f = 0; f < F.rows(); f++)
    {
        ...
        {
            Gijv.emplace_back(f + d * m, d2nd[F(f, 1)], eperp13(f, d));
            Gijv.emplace_back(f + d * m, d2nd[F(f, 0)], -eperp13(f, d));
            Gijv.emplace_back(f + d * m, d2nd[F(f, 2)], eperp21(f, d));
            Gijv.emplace_back(f + d * m, d2nd[F(f, 0)], -eperp21(f, d));
        }
    }
 ...
 void recompute_dxdy(SCAFData& s) {
   ...
   // s.m_V = new_V;
    ...
    igl::local_basis(new_V, new_F, F1, F2, F3);
    ...
    Eigen::SparseMatrix<double> G,G_1;



    grad_tri_merge(new_V, new_F,G,false, d2nd,s.m_V.rows());
 ...
 double compute_energy(SCAFData& s, const Eigen::MatrixXd& w_uv, bool whole)
{
    ...
    double energy = 0;
    if (s.Ji_m.rows() != 0) {
        energy += igl::mapping_energy_with_jacobians(s.Ji_m, s.m_M, s.slim_energy, 0);
    }
    else {
        // energy += compute_suw_uvrface_ARAP_energy(V_new);
        compute_energy_from_jacobians(s.Ji_m, s.m_M, s.slim_energy);
    }
    if (whole)
 ...
 void build_scaffold_linear_system(const SCAFData &s, Eigen::SparseMatrix<double> &L, Eigen::VectorXd &rhs)
{
 ...
 void build_weighted_arap_system(SCAFData &s, Eigen::SparseMatrix<double> &L, Eigen::VectorXd &rhs)
{
  // fixed frame solving:
 ...
 double perform_iteration(SCAFData &s)
{
  ...
  //// update m_M
  //Eigen::VectorXd M;
  //igl::doublearea(V_out.topRows(s.mf_num), s.w_T, M);
  //s.m_M.conservativeResize(s.mf_num + M.size());
  //M /= M.sum();
  //M *= M.rows();
  //for (int i = 0; i < M.size(); i++) {
...
namespace RIGIDT {
        ...
        double soft_p)
    {
        Eigen::MatrixXd CN;
        Eigen::MatrixXi FN;
        add_new_patch(data, V, F, Eigen::RowVector2d(0, 0), V_init);
    ...
    Eigen::MatrixXd scaf_solve(SCAFData& s, int iter_num)
    {
        using namespace std;
        using namespace Eigen;
        s.energy = compute_energy(s, s.w_uv, false) / s.mesh_measure;
        ...
        for (int it = 0; it < iter_num; it++)
        {
            ...
            s.total_energy = compute_energy(s, s.w_uv, true) / s.mesh_measure;
            s.rect_frame_V = Eigen::MatrixXd();
            mesh_improve(s);

            double new_weight = s.mesh_measure * s.energy / (s.sf_num * 100000);
            s.scaffold_factor = new_weight;
            update_scaffold(s);

            s.total_energy = perform_iteration(s);

            s.energy =
                RIGIDT::compute_energy(s, s.w_uv, false) / s.mesh_measure;
        }
    ...
    {
        s.energy = RIGIDT::compute_energy(s, s.w_uv, false) / s.mesh_measure;

        s.total_energy = RIGIDT::compute_energy(s, s.w_uv, true) / s.mesh_measure;
        s.rect_frame_V = Eigen::MatrixXd();
        mesh_improve(s);
        ...
        compute_jacobians(s, s.w_uv, true);
        igl::slim_update_weights_and_closest_rotations_with_jacobians(s.Ji_m, s.slim_energy, 0, s.W_m, s.Ri_m);
        igl::slim_update_weights_and_closest_rotations_with_jacobians(s.Ji_s, s.scaf_energy, 0, s.W_s, s.Ri_s);
