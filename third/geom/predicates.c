...
REAL exactinit ()
{
	...
	if (!implementCount) {
		...
		splitter = 1.0;
		check = 1.0;
		/* Repeatedly divide `epsilonilon' by two until it is too small to add to    */
		...
		macheps = epsilon; //added by Jeff 
	}
	if (implementCount > 1) {
		//cant be invoked more than once!
		exit(-2);
	}
...
int compression(elen, e, h)                         /* e and h may be the same. */
int elen;
REAL *e;
REAL *h;
{
